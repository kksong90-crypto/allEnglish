...
(function installVocaStationStudentUi(global) {
  ...
  async function openVocaBookById(encodedBookId, startDay, endDay) {
    ...
    const numericEnd = Number(endDay || startDay);
    const hasAssignedRange = Number.isFinite(numericStart) && Number.isFinite(numericEnd) && numericStart > 0 && numericEnd >= numericStart;
    if (hasAssignedRange) {
      const assignedIds = sortedSections(book).filter(section => {
        const sectionNo = Number(section.sectionNo);
        return Number.isFinite(sectionNo) && sectionNo >= numericStart && sectionNo <= numericEnd;
      }).map(section => text(section.sectionId)).filter(Boolean);
      if (!assignedIds.length) {
        alert('현재 배정된 Day를 공개 단어장에서 찾을 수 없습니다. 선생님에게 연결 확인을 요청해 주세요.');
        return;
      }
      state.activeScopeSectionIds = new Set(assignedIds);
    } else {
      state.activeScopeSectionIds = null;
    }
