...
const context = vm.createContext({
  ...
  encodeURIComponent,
  decodeURIComponent,
  currentUser: { id: '2011001', studentId: '2011001', token: 'student-token' },
  ...
  apiPost: async payload => {
    ...
    if (payload.action === 'getVocaCatalog') return {
      success: true,
      books: [
        { bookId: 'BOOK-PUBLIC', revisionId: 'REV-1', displayName: '수능완성', wordCount: 50, studentVisible: true, sections: [{ sectionId: 'SEC-1', sectionNo: '1', label: 'Day 1', sortOrder: 1, wordCount: 50 }] },
  ...
  apiGet: async payload => {
    ...
    if (payload.action === 'getMyLearningSchedule') return {
      ...
      days: [{ date: '2099-08-10', vocab: [
        { planType: 'REGULAR', vocabBookId: 'BOOK-PUBLIC', regularStartDay: 1, regularEndDay: 1, regularText: 'Day 1' },
